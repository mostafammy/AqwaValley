...
async def run_test():
    ...
    try:
        ...
        # -> Fill the national id and password fields and submit the login form (use indexes 83, 92, then click 94).
        ...
        # -> Fill national id (index 237), fill password (index 245), then click submit (index 247).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/main/div[2]/div/div[2]/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('30606151600913')
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Please select a well to view details').first).to_be_visible(timeout=3000)
