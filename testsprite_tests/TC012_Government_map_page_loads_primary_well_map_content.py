...
async def run_test():
    ...
    try:
        ...
        # -> Fill the national-id field with the provided username, fill the password, then submit the login form.
        ...
        # -> Fill the national-id and password fields and submit the login form (attempt 2), then wait for the app to render the dashboard so the map view link can be located.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/main/div[2]/div/div[2]/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('30606151600913')
        ...
        await expect(frame.locator('text=Well markers').first).to_be_visible(timeout=3000)
