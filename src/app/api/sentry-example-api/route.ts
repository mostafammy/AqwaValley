...
export async function GET(request: Request) {
  const envAllow = process.env.NODE_ENV === 'development' || process.env.ALLOW_SENTRY_EXAMPLE === 'true'
  const adminToken = process.env.SENTRY_EXAMPLE_ADMIN_TOKEN
  const headerToken = request.headers.get('x-admin-token') ?? request.headers.get('X-Admin-Token') ?? undefined
  const tokenAllow = Boolean(adminToken && headerToken && adminToken === headerToken)
