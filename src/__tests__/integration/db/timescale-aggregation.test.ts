...
describe("TimescaleDB aggregation contract (Invariant #9)", () => {
  it("timescaledb_time_bucket_groups_boundary_rows_correctly", () => {
    ...
    expect(analyticsSource).toContain(
      "timestamp BETWEEN ${input.from.toISOString()}::timestamptz",
    );
    expect(analyticsSource).toContain(
  ...
  it("timescaledb_chunk_boundary_queries_do_not_double_count", () => {
    const metricsRouteSource = readSource(
      "src/app/api/wells/[id]/metrics/route.ts",
    );

    expect(metricsRouteSource).toContain(
      'time_bucket(${bucketMinutes.toString() + " minutes"}::interval, sd.timestamp) AS bucket',
    );
    expect(metricsRouteSource).toContain(
      "sd.timestamp >= NOW() - (${rangeHours.toString()} || ' hours')::interval",
    );
